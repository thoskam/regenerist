...
## 🚧 Current Sprint: Phase 1 - Multi-Tenancy & Authentication

**Status:** In Progress

We are adding user authentication and multi-tenancy to support multiple users and campaigns. This involves:

1. **Authentication System** - NextAuth.js with Discord OAuth (primary) + Google
2. **Database Schema Expansion** - Adding User, Campaign, CampaignMember, CharacterCampaign tables
3. **User Features** - Profile page, "My Characters" filtering, character visibility permissions
4. **Campaign Management** - Create/join campaigns, invite links, DM dashboard

### New Tables Being Added
```prisma
model User {
  id visually (omitted for brevity, but you'll add)
  - discordId, discordUsername, email, avatar, createdAt
  - Relations: characters, campaignsOwned, campaignMemberships
}

model Campaign {
  - id, name, inviteCode, dmUserId, createdAt
  - Relations: dm (User), members, characters
}

model CampaignMember {
  - userId, campaignId, role ('dm' | 'player')
}

model CharacterCampaign {
  - characterId, campaignId (join table)
}
```

### Migration Strategy
- Existing characters will be assigned to the first user who logs in
- Characters without users should still be accessible (backward compatibility during migration)
...
**Stack:** Next.js 14 (App Router) + TypeScript + Prisma + PostgreSQL + AWS Bedrock + NextAuth.js (coming)

### Key Directories

- `app/` - Next.js App Router pages and API routes
- `app/api/auth/` - NextAuth.js authentication routes (coming)
...
- **Character** - The persistent soul/identity (has name, slug, level, **isRegenerist flag**)
  ...
  - `userId` - Foreign key to User (being added)
...
**Tables Being Added (4):**
- **User** - Authentication and profile data (Discord OAuth)
- **Campaign** - D&D campaigns/groups with DM ownership
- **CampaignMember** - User membership in campaigns with role
- **CharacterCampaign** - Links characters to campaigns
...
# Authentication (Phase 1 - being added)
...
- Session available via `getServerSession()` in API routes

### Character Management
- `GET /api/characters` - List all characters (will be scoped to user after Phase 1)
...
### Campaign Management (Coming in Phase 1)
- `GET /api/campaigns` - List user's campaigns
- `POST /api/campaigns` - Create campaign (creator becomes DM)
- `GET /api/campaigns/[id]` - Campaign details with members and characters
- `POST /api/campaigns/[id]/join` - Join campaign via invite code
...
After Phase 1, the roadmap includes:
- **Phase 2**: Actions, Combat & Rest System
- **Phase 3**: Dice Rolling with Discord integration
- **Phase 4**: Inventory & Equipment with Global Modifier Engine
- **Phase 5-9**: Spellcasting enhancements, Feats, UI/UX, Integrations, DM Tools
