...
import { randomClass, randomRace, randomEffect, generateCharacterName } from '@/lib/randomizer'
...
export async function POST(request: Request) {
  try {
    ...
    // Negative final levels treated as 1
    const levelRoll = rollD4() - 2
    const newLevel = Math.max(1, currentLevel - levelRoll)
    const level = newLevel

    // Get the current active life to mark it inactive and for story context
